...
def root_v60():
    ...
    return HTMLResponse(html,headers={
      ...
      'Pragma':'no-cache',
      'Expires':'0',
      'X-Plugy-Version':'60.0',
      'X-Plug-Art-UI':'glass-dashboard'
    })

@app.middleware('http')
async def v60_headers(request:Request,call_next):
    response=await call_next(request)
    p=request.url.path
    if p in ('/','/index.html','/static/PLUGY_final_animated.glb') or 'plugy_v59_polished' in p or 'ui_v60_glass' in p:
...
def status_v59():
    ...
    return {
      ...
      'ui_runtime':'ui_v60_glass.js' in html,
...
print(f"PLUG_ART_V60_READY bytes={GLB.stat().st_size if GLB.exists() else 0} ui=glass_dashboard navigation=simplified design={RESULT.get('design')}",flush=True)
