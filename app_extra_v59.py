...
def root_v102(request:Request):
    ...
    headers={
      ...
      'ETag':etag,
      'X-Plug-Art-Version':'120.0',
      'X-Plug-Art-UI':'plug-art-v120-bureau-workspace'
    }
...
def status_v90():
    ...
    return {
      ...
      'version':'120.0',
      'ui':'plug-art-v120-bureau-workspace',
      'reference_direction':'V120 PLUG ART: compact internal work cockpit with a three-mode Bureau for documents, reusable templates and Open Call application packages, plus CRM outreach, actionable Today queue, PLUGY operator, Instagram Studio and mobile-first workflows',
      ...
      'background':'compact PLUG ART internal workspace with lightweight dashboard-first loading, modular operational data, structured Bureau and integrated content studio'
    }

print(f"PLUG_ART_V120_READY ui=internal_dashboard bureau=persistent prospection=crm open_call_workflow=on drafts=persistent realistic=on contextual_motion=on plugy=single_drawer instagram=control_center command_palette=on sidebar=adaptive graph={_ig_graph_version()} instagram_configured={_ig_configured()} studio=instagram_queue voice=streaming internal=on plugy_bytes={GLB.stat().st_size if GLB.exists() else 0}",flush=True)
