...
app=v42.app;app.version="51.0";BASE=Path(__file__).resolve().parent;GLB=BASE/"static"/"plugy.glb";INDEX=BASE/"static"/"index.html"
...
if INDEX.exists():
 ...
 scripts='\n'.join(f'<script defer src="/static/{name}?v=51.20260916.0"></script>' for name in names);page=page.replace("</body>",scripts+"\n</body>",1);INDEX.write_text(page,encoding="utf-8")
print(f"PLUGY_V51_READY bytes={len(raw)} nodes={result.get('nodes')} animations={','.join(result.get('animations',[]))} design={result.get('design')} sha256={digest}",flush=True);print("PLUG_ART_STUDIO_V51_READY plugy=expressive_head_premium_assistant animated=8 clean_ui=on",flush=True)
@app.middleware("http")
