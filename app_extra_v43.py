...
app=v42.app;app.version="45.0";BASE=Path(__file__).resolve().parent;GLB=BASE/"static"/"plugy.glb";INDEX=BASE/"static"/"index.html"
...
if INDEX.exists():
 ...
 scripts='\n'.join(f'<script defer src="/static/{name}?v=45.20260915.0"></script>' for name in names);page=page.replace("</body>",scripts+"\n</body>",1);INDEX.write_text(page,encoding="utf-8")
print(f"PLUGY_V43_READY bytes={len(raw)} nodes={result.get('nodes')} animations={','.join(result.get('animations',[]))} sha256={digest}",flush=True);print("PLUG_ART_STUDIO_V45_READY clean_ui=on simple_advanced=on contextual_actions=on preview_large=on",flush=True)
@app.middleware("http")
async def v45_no_cache(request:Request,call_next):
