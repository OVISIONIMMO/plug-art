...
function prefetchRouteV164(id){
  ...
  if(id==='creation')ensureCreationCssV163().catch(()=>{});
  if(id==='ideas')api('/api/v156/ideas',{cacheTtl:30000}).catch(()=>{});
  if(id==='bureau'){
    api('/api/v156/bureau/files?project=millenaire',{cacheTtl:30000}).catch(()=>{});
...
function bindRoutePrefetchV164(){
  ...
  $('a[href="/plugy"]').forEach(el=>el.addEventListener('pointerenter',()=>{if(!document.querySelector('link[data-prefetch-plugy-v164]')){const l=document.createElement('link');l.rel='prefetch';l.href='/plugy';l.dataset.prefetchPlugyV164='1';document.head.appendChild(l)}},{once:true,passive:true}));
}
...
function playMotion(name='Idle',loop=false){
  ...
  const run=()=>{
    ...
    if(['Blink','DoubleBlink','Wink','SoftEyes','EyeThink','ArmThink'].includes(name))name=(a.includes('Think')?'Think':'Idle');
...
$('#plugyModel')?.addEventListener('pointerenter',()=>{
  ...
  if(mini)return;
  const hello=choosePlugyMotion(['ArmHello','Curious','Attentive']);
...
$('#plugyModel')?.addEventListener('dblclick',()=>{
  openPlugy();
  const react=choosePlugyMotion(['ArmExplain','Attentive','Present']);
...
function createCarouselForOpportunity(id){return createPublicationFromOpportunityV162(id);}
...
async function streamPlugyRequest(payload,onDelta){
  ...
  try{
    ...
    while(true){
      const {value,done}=await reader.read();arm();
...
function createProgressiveSpeaker(){
  ...
  let spoken=0,queued=0,finished=false,watchdog=null;
  const stopWatch=()=>{if(watchdog){clearInterval(watchdog);watchdog=null}};
  const finish=()=>{if(!finished||queued>0)return;stopWatch();state.voiceReply=false;$('#plugyState span').textContent='Prêt';playMotion('Idle',true);resumeConversationListening(430)};
  const pieces=text=>{
    text=clean(text);const out=[];
    while(text.length>190){let cut=Math.max(text.lastIndexOf('. ',190),text.lastIndexOf(', ',190),text.lastIndexOf(' ',190));if(cut<80)cut=190;out.push(text.slice(0,cut+1));text=text.slice(cut+1)}
    if(text)out.push(text);return out;
  };
  const enqueue=text=>{
    pieces(text).forEach(part=>{
      queued++;const u=new SpeechSynthesisUtterance(part);u.lang='fr-FR';u.rate=1.13;u.pitch=1;u.volume=1;const voice=plugySpeechVoice||resolvePlugySpeechVoice();if(voice)u.voice=voice;
    ...
    if(!watchdog)watchdog=setInterval(()=>{try{if(queued>0&&!speechSynthesis.speaking)speechSynthesis.resume();else if(queued>0)speechSynthesis.resume()}catch{}},3500);
  };
  return {
    push(full,final=false){
      const all=String(full||''),pending=all.slice(spoken);if(!pending&&!final)return;
      if(final){spoken=all.length;enqueue(pending);finished=true;finish();return}
...
async function askPlugy(message,injectTarget=null){
  ...
  if(!injectTarget&&looksLikePlugyVisualV161(message))return generatePlugyVisualV161(message);
  ...
  let answer='',received=false;const progressiveVoice=null;
  const append=delta=>{
...
function bindOppActions(root=document){
  ...
  $('[data-opp-create]',root).forEach(b=>b.onclick=()=>createPublicationFromOpportunityV162(Number(b.dataset.oppCreate)));
...
function renderArtists(){
  installArtistV161();
  const box=$('#artistGrid'),all=state.bootstrap?.artists||[],term=clean($('#artistSearchV161')?.value).toLowerCase(),discipline=clean($('#artistFilterV161')?.value).toLowerCase();
  ...
  $$('[data-artist-profile]',box).forEach(card=>card.onclick=e=>{if(e.target.closest('button,a'))return;openArtistProfileV161(Number(card.dataset.artistProfile))});
  $$('[data-artist-contact]',box).forEach(b=>b.onclick=e=>{e.stopPropagation();createArtistContactV161(Number(b.dataset.artistContact))});
  $$('[data-artist-plugy]',box).forEach(b=>b.onclick=e=>{e.stopPropagation();const a=all.find(x=>String(x.id)===b.dataset.artistPlugy);askPlugy('Analyse le profil de '+clean(a?.name)+'. Discipline : '+clean(a?.discipline)+'. Propose des expositions et prochaines actions concrètes, sans concours ni prix.')});
}
...
let plugyNaturalAudio=null;
function speakPlugyBrowserFallback(text){
  if(!('speechSynthesis' in window)){state.voiceReply=false;return}
  try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='fr-FR';u.rate=1.06;u.pitch=.98;u.volume=1;const voice=plugySpeechVoice||resolvePlugySpeechVoice();if(voice)u.voice=voice;
    u.onstart=()=>{$('#plugyState span').textContent='Parle…';playMotion('Speak',true)};
    u.onend=()=>{$('#plugyState span').textContent='Prêt';state.voiceReply=false;playMotion('Idle',true);resumeConversationListening(380)};
    u.onerror=()=>{state.voiceReply=false;playMotion('Idle',true);resumeConversationListening(600)};speechSynthesis.speak(u)
  }catch{state.voiceReply=false}
}
async function speakPlugy(text){
  text=clean(text);if(!text){state.voiceReply=false;return}
  try{
    if(plugyNaturalAudio){try{plugyNaturalAudio.pause()}catch{};plugyNaturalAudio=null}
    ...
    const blob=await r.blob(),url=URL.createObjectURL(blob),audio=new Audio(url);plugyNaturalAudio=audio;audio.preload='auto';
    audio.onplay=()=>{$('#plugyState span').textContent='Parle…';playMotion('Speak',true)};
    const done=()=>{URL.revokeObjectURL(url);plugyNaturalAudio=null;state.voiceReply=false;$('#plugyState span').textContent='Prêt';playMotion('Idle',true);resumeConversationListening(320)};
    audio.onended=done;audio.onerror=()=>{URL.revokeObjectURL(url);plugyNaturalAudio=null;speakPlugyBrowserFallback(text)};
    await audio.play();
  }catch{speakPlugyBrowserFallback(text)}
}
...
function setCreationPreviewZoom(){
  const scale=Math.max(.45,Math.min(1.5,Number($('#creationZoom')?.value||1)));
...
async function generateCarouselVariants(){
  ...
  const buttons=[$('#studioGenerateVariants'),$('#stageGenerateVariants')].filter(Boolean);
  if(buttons.some(b=>b.disabled))return;
  buttons.forEach(b=>{b.disabled=true;b.dataset.oldLabel=b.textContent;b.textContent='✦ Génération…'});
  ...
  const directions=[
    ['Éditorial','editorial','composition magazine culturel premium, lignes fines, grille asymétrique, espace négatif, détail graphique fort, direction artistique de revue'],
    ['Affiche','poster','affiche contemporaine de centre d’art, formes géométriques, contraste maîtrisé, matière papier ou encre, composition audacieuse'],
    ['Graphique','graphic','système visuel abstrait sophistiqué, traits, blocs, rythme, architecture graphique, aucun décor de galerie nécessaire']
  ];
  const jobs=directions.map(async([label,style,direction])=>{
    const out=await api('/api/v32/content/image',{method:'POST',timeout:115000,body:JSON.stringify({prompt:base+'. '+direction+'. Aucun texte lisible, aucune lettre, aucun logo.',style,ratio,quality:'medium'})});
    if(!out?.url)throw new Error('Aucun visuel retourné');
    return {url:out.url,label:label+' · '+new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}),style};
  });
  const results=await Promise.allSettled(jobs),made=results.filter(x=>x.status==='fulfilled').map(x=>x.value);
  made.forEach(x=>{state.visual.history=state.visual.history||[];state.visual.history.unshift(x)});
  if(made.length){
    state.visual.url=made[0].url;s.image=made[0].url;
    const d=slideDesign(s);if(d.layout==='minimal')d.layout='editorial';d.imageOpacity=Math.max(86,Number(d.imageOpacity||0));
    renderCarousel();scheduleDraftAutosave();
  }
  buttons.forEach(b=>{b.disabled=false;b.textContent=b.dataset.oldLabel||'✦ 3 variantes';delete b.dataset.oldLabel});
  renderStudioImages();renderVisualHistory();
  const failed=results.length-made.length;
  toast(made.length?(made.length+' variantes prêtes'+(failed?' · '+failed+' indisponible':'')):'Génération indisponible');
}
...
function drawEditorialPattern(ctx,w,h,pattern,accent){
  ctx.save();ctx.globalAlpha=.9;ctx.strokeStyle=accent;ctx.fillStyle=accent;ctx.lineWidth=Math.max(2,w*.004);
  if(pattern==='grid'){
    ctx.globalAlpha=.18;for(let x=w*.08;x<w;x+=w*.12){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}for(let y=h*.08;y<h;y+=h*.1){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}
    ctx.globalAlpha=.8;ctx.fillRect(w*.08,h*.1,w*.22,h*.012);
  }else if(pattern==='ribbon'){
    ctx.globalAlpha=.72;ctx.lineWidth=w*.035;ctx.beginPath();ctx.moveTo(-w*.1,h*.26);ctx.bezierCurveTo(w*.28,h*.04,w*.55,h*.48,w*1.08,h*.16);ctx.stroke();
    ctx.globalAlpha=.18;ctx.lineWidth=w*.11;ctx.beginPath();ctx.moveTo(w*.08,h*.72);ctx.bezierCurveTo(w*.34,h*.5,w*.64,h*.92,w*.98,h*.58);ctx.stroke();
  }else if(pattern==='blocks'){
    ctx.globalAlpha=.78;ctx.fillRect(w*.67,h*.08,w*.23,h*.18);ctx.globalAlpha=.16;ctx.fillRect(w*.52,h*.28,w*.38,h*.26);ctx.globalAlpha=.3;ctx.fillRect(w*.08,h*.78,w*.32,h*.07);
  }else if(pattern==='signal'){
    ctx.globalAlpha=.78;for(let i=0;i<7;i++){ctx.fillRect(w*(.08+i*.055),h*(.12+i*.045),w*.012,h*(.18+i*.025))}
    ctx.globalAlpha=.2;ctx.beginPath();ctx.arc(w*.76,h*.28,w*.17,0,Math.PI*2);ctx.stroke();
  }else if(pattern==='frame'){
    ctx.globalAlpha=.72;ctx.strokeRect(w*.07,h*.06,w*.86,h*.88);ctx.globalAlpha=.24;ctx.strokeRect(w*.11,h*.1,w*.34,h*.32);ctx.fillRect(w*.72,h*.08,w*.16,h*.012);
  }else{
    ctx.globalAlpha=.62;ctx.beginPath();ctx.arc(w*.77,h*.23,w*.18,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=.2;ctx.lineWidth=w*.08;ctx.beginPath();ctx.arc(w*.72,h*.28,w*.28,0,Math.PI*1.55);ctx.stroke();ctx.globalAlpha=.7;ctx.fillRect(w*.08,h*.13,w*.24,h*.012);
  }
  ctx.restore();
}
...
async function renderCarouselSlideBlob(index){
  ...
  if(!img&&d.layout!=='minimal')drawEditorialPattern(ctx,w,h,d.pattern||'orbit',slideAccentColor(d.accent));
  ...
  const exportFamilies={Space:'"Space Grotesk"',Inter:'Inter',System:'Arial',Serif:'Georgia',Mono:'monospace'},exportFamily=exportFamilies[d.fontFamily]||exportFamilies.Space,exportWeight=Number(d.fontWeight||700),pixelScale=w/620;
  try{ctx.letterSpacing=(Number(d.letterSpacing||0)*pixelScale).toFixed(2)+'px'}catch{}
  ...
  ctx.fillStyle=ink;ctx.font=exportWeight+' '+Math.max(18,Math.round(Number(d.labelPx||11)*pixelScale*scaleText))+'px '+exportFamily;ctx.fillText(String(s.kicker||'PLUG ART').toUpperCase(),x,titleY-Math.round(42*scaleText),maxW);
  ctx.font=exportWeight+' '+Math.max(32,Math.round(Number(d.titlePx||46)*pixelScale*scaleText))+'px '+exportFamily;
  const titleFontPx=Math.max(32,Math.round(Number(d.titlePx||46)*pixelScale*scaleText)),titleLines=canvasTextLines(ctx,s.title||'Sans titre',maxW,d.layout==='split'?6:4),titleLH=Math.round(titleFontPx*Number(d.lineHeight||1.08));
  ...
  const bodyFontPx=Math.max(16,Math.round(Number(d.bodyPx||15)*pixelScale*scaleText));ctx.font=Math.min(exportWeight,600)+' '+bodyFontPx+'px '+exportFamily;ctx.fillStyle=d.theme==='night'?'#d7d9e1':'#343741';
  const bodyLines=canvasTextLines(ctx,s.body||'',maxW,5),bodyLH=Math.round(bodyFontPx*(Number(d.lineHeight||1.08)+.28));
  bodyLines.forEach((line,i)=>ctx.fillText(line,x,bodyY+i*bodyLH,maxW));
  ctx.fillStyle=ink;ctx.font=exportWeight+' '+Math.max(16,Math.round(Number(d.ctaPx||12)*pixelScale*scaleText))+'px '+exportFamily;
  ...
  for(const l of slideLayers(s)){
    ...
    if(l.type==='text'){ctx.fillStyle=l.color||'#111318';ctx.textAlign=l.align||'left';ctx.textBaseline='top';const layerFamilies={Inter:'Inter',Space:'"Space Grotesk"',Serif:'Georgia',Mono:'monospace'};ctx.font=String(l.weight||700)+' '+Math.max(12,Number(l.size||24)*w/1080)+'px '+(layerFamilies[l.fontFamily]||'Inter');const lines=canvasTextLines(ctx,l.text||'',lw,6);lines.forEach((line,i)=>ctx.fillText(line,lx+(l.align==='center'?lw/2:0),ly+i*Math.max(18,Number(l.size||24)*1.18*w/1080),lw))}
...
function openOpportunity(id){
  ...
  renderOpenCallFavorite();
  const actions=$('#callDrawer .call-actions');
  if(actions){
    let createBtn=$('#callCreatePublicationV162');
    if(!createBtn){createBtn=document.createElement('button');createBtn.id='callCreatePublicationV162';createBtn.className='primary';createBtn.textContent='✦ Créer une publication';actions.prepend(createBtn)}
    createBtn.onclick=()=>{$('#callDrawer').classList.remove('open');createPublicationFromOpportunityV162(id)};
  }
  $('#callDrawer').classList.add('open');playMotion('Curious');
...
{id:'black-museum',name:'Black Museum',sub:'Premium sombre',layout:'poster',theme:'night',accent:'pink',pattern:'signal',bg:'#111218',preview:'radial-gradient(circle at 72% 28%,#e267ac 0 12%,transparent 13%),linear-gradient(145deg,#101116,#292839)'},
{id:'violet-depth',name:'Violet Depth',sub:'3D doux',layout:'split',theme:'soft',accent:'violet',pattern:'orbit',bg:'#ece8f8',preview:'radial-gradient(circle at 35% 40%,#725be5 0 19%,#c6b7ed 20% 34%,#f2edf8 35%)'},
{id:'cyan-grid',name:'Cyan Grid',sub:'Tech culturel',layout:'editorial',theme:'ultra',accent:'cyan',pattern:'grid',bg:'#effbfb',preview:'linear-gradient(90deg,transparent 48%,#9fe0e4 49% 51%,transparent 52%),linear-gradient(#effbfb,#dff8f9)'},
{id:'rose-object',name:'Rose Object',sub:'Designer toy',layout:'split',theme:'soft',accent:'pink',pattern:'orbit',bg:'#fff0f6',preview:'radial-gradient(circle at 60% 42%,#e987bc 0 20%,#f5bfd9 21% 34%,#fff0f6 35%)'},
{id:'red-poster',name:'Red Poster',sub:'Affiche impact',layout:'poster',theme:'ultra',accent:'red',pattern:'blocks',bg:'#f4ede8',preview:'linear-gradient(155deg,#efeee9 0 53%,#d9494e 54% 77%,#15161a 78%)'},
{id:'archive',name:'Archive',sub:'Photo / document',layout:'editorial',theme:'editorial',accent:'black',pattern:'grid',bg:'#eee9df',preview:'linear-gradient(135deg,#d8d1c5,#f5f1e9 45%,#77736c 46% 49%,#faf7f1 50%)'},
{id:'gallery-card',name:'Gallery Card',sub:'Expo minimal',layout:'minimal',theme:'editorial',accent:'black',pattern:'frame',bg:'#ffffff',preview:'linear-gradient(145deg,#fff 0 70%,#e8e8e8 70%)'},
...
function installCreationV161(){
 ...
 const rail=document.createElement('section');rail.id='creationV161Rail';rail.className='creation-v161-rail';
 rail.innerHTML='<div class="creation-v161-rail-head"><div><small>DESIGN LIBRARY</small><strong>Choisis un style, vois le résultat immédiatement.</strong></div><div><button id="creationV161Generate">✦ PLUGY visuel</button><button id="creationV161Fit">Ajuster aperçu</button></div></div><div class="creation-v161-style-scroll">'+CREATION_V161_STYLES.map(s=>'<button data-v161-style="'+s.id+'"><i style="--preview:'+s.preview+'"></i><span><b>'+esc(s.name)+'</b><small>'+esc(s.sub)+'</small></span></button>').join('')+'</div>';
 panel.insertAdjacentElement('beforebegin',rail);
 $$('[data-v161-style]',rail).forEach(b=>b.onclick=()=>applyCreationStyleV161(b.dataset.v161Style));
 $('#creationV161Fit').onclick=()=>fitStudioCanvas();
 $('#creationV161Generate').onclick=()=>{openPlugy();const input=$('#plugyInput');if(input){input.value='Génère-moi un visuel premium pour le design actuellement ouvert dans le Labo Création, sans texte lisible.';input.focus()}};
 requestAnimationFrame(()=>{fitStudioCanvas();renderCreationV161History()});
}
function applyCreationStyleV161(id){
 const p=CREATION_V161_STYLES.find(x=>x.id===id);if(!p)return;
 if(!state.carousel.slides.length)state.carousel.slides=makeMarketingSlides('open-call');
 const s=state.carousel.slides[state.carousel.active]||state.carousel.slides[0];if(!s)return;
 pushCreationHistory();const d=slideDesign(s);Object.assign(d,{layout:p.layout,theme:p.theme,accent:p.accent,pattern:p.pattern,backgroundColor:p.bg,fontScale:100,radius:24});
 setCreationMode('carousel');renderCarousel();scheduleDraftAutosave();$$('[data-v161-style]').forEach(b=>b.classList.toggle('active',b.dataset.v161Style===id));
}
function renderCreationV161History(){
...
async function generatePlugyVisualV161(message){
 if(state.plugyBusy){toast('PLUGY génère déjà');return}openPlugy();setPlugyBusy(true);addMsg(message,'user');playMotion('Charge',true);$('#plugyState span').textContent='Génération visuelle…';const wait=addMsg('Je construis le visuel…','bot');
 try{
  const ratio=state.view==='creation'?(state.carousel.format||$('#visualRatio')?.value||'4:5'):'4:5';
  const r=await api('/api/v32/content/image',{method:'POST',timeout:120000,body:JSON.stringify({prompt:message+' Direction PLUG ART premium, composition contemporaine, profondeur maîtrisée, rendu crédible et haut de gamme. Aucun texte lisible, aucun logo, aucun watermark.',style:'editorial',ratio,quality:'medium'})});
...
function installArtistV161(){
 const view=$('#view-network');if(!view)return;if(!$('#artistToolbarV161')){const bar=document.createElement('div');bar.id='artistToolbarV161';bar.className='artist-toolbar-v161';bar.innerHTML='<div><small>RÉSEAU ARTISTES</small><strong>Profils, contacts et opportunités adaptées.</strong></div><label><span>⌕</span><input id="artistSearchV161" placeholder="Rechercher artiste, ville, discipline…"></label><select id="artistFilterV161"><option value="">Toutes disciplines</option><option value="photo">Photographie</option><option value="peinture">Peinture</option><option value="sculpt">Sculpture</option><option value="digital">Digital</option></select><button id="artistAddV161">＋ Artiste</button>';view.prepend(bar);$('#artistSearchV161').oninput=renderArtists;$('#artistFilterV161').onchange=renderArtists;$('#artistAddV161').onclick=openArtistEditorV161}ensureArtistDrawerV161()
}
...
let creationPreviewResizeV162=null,creationLastAutoFitV162=0;
function ensureCreationPreviewV162(force=true){
  const panel=$('#carouselCreationPanel'),frame=$('.studio-canvas-frame'),canvas=$('#carouselCanvas');
  if(!panel||!frame||!canvas)return;
  if(!state.carousel.slides.length){
    state.carousel.slides=[{kicker:'PLUG ART',title:'Ta publication commence ici',body:'Choisis une opportunité ou écris ton message. Tout reste modifiable.',cta:'Découvrir →',image:'',image_prompt:'',design:{theme:'editorial',layout:'editorial',accent:'black',align:'left',fontScale:100,titleScale:100,bodyScale:100,labelScale:100,ctaScale:100,spacing:100,imageOpacity:64,radius:26,backgroundColor:'#f4f3ef',pattern:'frame'}}];
    state.carousel.active=0;renderCarousel();
  }
  const now=Date.now();
  if(force||now-creationLastAutoFitV162>800){creationLastAutoFitV162=now;fitStudioCanvas()}
  if(!creationPreviewResizeV162&&window.ResizeObserver){
    creationPreviewResizeV162=new ResizeObserver(()=>{if(state.view==='creation'&&state.creationMode==='carousel')fitStudioCanvas()});
    creationPreviewResizeV162.observe(frame);
  }
...
async function createPublicationFromOpportunityV162(id){
  ...
  try{await ensureViewData('creation')}catch{}
  setCreationMode('carousel');
  const facts=[o.title,o.summary||o.radar_reason,[o.city,o.country].filter(Boolean).join(' · '),o.deadline?'Deadline : '+o.deadline:'',o.fee?'Frais : '+o.fee:''].filter(Boolean).join('\n');
  if($('#contentSource'))$('#contentSource').value=String(id);
  if($('#carouselSource'))$('#carouselSource').value=String(id);
  ...
  $('#creationV161Rail')?.insertAdjacentElement('afterend',chip);
  $('#creationOpportunityGenerateV162')?.addEventListener('click',()=>$('#carouselGenerate')?.click());
  $('#creationOpportunityDetailV162')?.addEventListener('click',()=>{route('opencalls');setTimeout(()=>openOpportunity(id),50)});
  persistWorkflow(id,{workflow_status:'drafting',next_action:'Finaliser la publication'}).catch(()=>{});
  toast('Opportunité chargée dans le Labo');return true;
}
function installCreationV162(){
  const panel=$('#carouselCreationPanel');if(!panel)return;
  ensureCreationPreviewV162(true);
  const stage=panel.querySelector('.studio-stage');if(stage&&!$('#creationPreviewBadgeV162')){
    const badge=document.createElement('div');badge.id='creationPreviewBadgeV162';badge.className='creation-preview-badge-v162';
...
let projectLibrarySeedPromiseV164=null;
async function ensureProjectLibrarySeedV164(project=''){
  if(projectLibrarySeedPromiseV164)return projectLibrarySeedPromiseV164;
  const key='plugart:v164:library-seeded';
  if(sessionStorage.getItem(key)==='1')return true;
  projectLibrarySeedPromiseV164=api('/api/v164/projects/library/seed',{
    method:'POST',timeout:20000,body:JSON.stringify({project:project||''})
  }).then(async r=>{
    sessionStorage.setItem(key,'1');
    if(state.dataLoaded.bureau){
      try{state.bureau=await api('/api/v107/bureau',{noMemCache:true,cacheTtl:0});renderBureauFolders();if(state.view==='bureau'&&state.bureauMode==='documents')renderBureau()}catch{}
    }
    return r;
  }).catch(e=>{projectLibrarySeedPromiseV164=null;throw e});
  return projectLibrarySeedPromiseV164;
}
...
/* ================= V163 IDEA CLOUD ================= */
function installIdeasV163(){
  const refresh=$('#ideasRefresh');
  if(refresh?.dataset.bound==='1')return;
  if(refresh){refresh.dataset.bound='1';refresh.onclick=()=>loadIdeasV163(true)}
  $('#ideasNew')?.addEventListener('click',()=>createIdeaV163(state.ideaProject||''));
  $('#ideasProjectFilter')?.addEventListener('change',e=>{state.ideaProject=e.target.value||'';renderIdeasV163()});
  $$('[data-idea-project]').forEach(b=>b.onclick=()=>{state.ideaProject=b.dataset.ideaProject||'';if($('#ideasProjectFilter'))$('#ideasProjectFilter').value=state.ideaProject;renderIdeasV163()});
  $('#ideasToBureau')?.addEventListener('click',()=>{state.bureauMode='projects';route('bureau')});
  $('#ideasPlugy')?.addEventListener('click',()=>openPlugy('Analyse mon Nuage à idées'+(state.ideaProject?' pour le projet '+projectLabelV163(state.ideaProject):'')+'. Regroupe les idées proches, fais ressortir les plus prometteuses et propose trois actions concrètes.'));
  loadIdeasV163(false);
}
...
function renderIdeasV163(){
  ...
  const rows=(state.ideas||[]).filter(i=>!project||(i.project||'')===project);
  if(!rows.length){cloud.innerHTML='<div class="idea-empty-v163"><b>✦</b><strong>Aucune idée ici</strong><span>Crée une première idée ou change de projet.</span><button id="ideaEmptyNewV163">＋ Nouvelle idée</button></div>';$('#ideaEmptyNewV163').onclick=()=>createIdeaV163(project);return}
  cloud.innerHTML=rows.map((i,idx)=>{
    const px=Number(i.pos_x),py=Number(i.pos_y),x=Number.isFinite(px)?px:12+(idx%4)*22,y=Number.isFinite(py)?py:10+Math.floor(idx/4)*23;
    return '<article class="idea-card-v163 tone-'+esc(i.color||'violet')+'" data-idea-id="'+i.id+'" style="left:'+Math.max(2,Math.min(82,x))+'%;top:'+Math.max(2,Math.min(78,y))+'%">'+
      '<button class="idea-drag-v163" aria-label="Déplacer">⋮⋮</button>'+
      '<small>'+esc(i.project?projectLabelV163(i.project):'IDÉE LIBRE')+'</small>'+
...
async function createIdeaV163(project=''){
  ...
  const body=prompt('Développe l’idée en quelques lignes','')||'';
  try{
    const i=await api('/api/v156/ideas',{method:'POST',body:JSON.stringify({
      title:clean(title),body,project:project||state.ideaProject||'',stage:'explore',
      color:['violet','cyan','rose','orange'][Math.floor(Math.random()*4)],
      pos_x:14+Math.random()*56,pos_y:10+Math.random()*52
    })});
...
async function editIdeaV163(id){
  ...
  const title=prompt('Titre',i.title||'');if(title===null)return;
  const body=prompt('Développement',i.body||'');if(body===null)return;
  const project=prompt('Projet : aubervilliers, millenaire, gennevilliers, chanteraines ou vide',i.project||'');if(project===null)return;
  try{
...
function bindIdeaDragV163(card){
  ...
  handle.onpointermove=e=>{
    ...
    const y=Math.max(1,Math.min(80,(e.clientY-rect.top)/Math.max(1,rect.height)*100));
