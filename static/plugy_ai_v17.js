(()=>{
  ...
  function settle(nodes,text,isError=false){
    nodes.forEach(el=>{
      ...
      if(isError){
        el.style.border='1px solid rgba(220,60,80,.24)';
        el.style.background='rgba(255,245,247,.95)';
      }
  ...
  async function robustAsk(q){
    ...
    if(!q)return;
    addMessage(q,'user');
    const floatChat=$('#floatChat');
    if(floatChat)floatChat.classList.add('open');
    const waiting=addMessage('PLUGY réfléchit…','bot',true);
    const controller=new AbortController();
    const timer=setTimeout(()=>controller.abort(),60000);
    try{
      const response=await fetch('/api/plugy',{
        method:'POST',
      ...
      if(!answer)throw new Error('PLUGY a reçu une réponse vide');
      settle(waiting,answer,false);
      console.info('[PLUGY]',{ai:data.ai,model:data.model,fallback:data.fallback||false});
    }catch(err){
      const message=err?.name==='AbortError'
        ? 'PLUGY met trop de temps à répondre. Réessaie dans quelques secondes.'
        : `PLUGY n’a pas pu répondre : ${err?.message||'erreur inconnue'}`;
      settle(waiting,message,true);
      console.error('[PLUGY]',err);
    }finally{
      clearTimeout(timer);
    }
  }
