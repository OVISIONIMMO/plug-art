...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v82_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v82_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v82_history',JSON.stringify(A.history))}catch{}}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v82_last_opp',A.lastOppId)}catch{}
}
...
function bindV82Viewer(mv){
  if(!mv||mv.dataset.v82Bound==='1')return;mv.dataset.v82Bound='1';
  const normal=()=>{try{mv.timeScale=1;mv.setAttribute('rotation-per-second','3.8deg')}catch{}};
  mv.addEventListener('pointerenter',()=>{try{mv.timeScale=1.18;mv.setAttribute('rotation-per-second','5.5deg')}catch{}});
...
bindV82Viewer(q('#plugyModel'));bindV82Viewer(q('#plugyFloatModel'));
