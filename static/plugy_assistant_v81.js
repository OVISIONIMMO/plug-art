...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v81_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v81_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v81_history',JSON.stringify(A.history))}catch{}}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v81_last_opp',A.lastOppId)}catch{}
}
...
function bindV81Viewer(mv){
  if(!mv||mv.dataset.v81Bound==='1')return;mv.dataset.v81Bound='1';
  const normal=()=>{try{mv.timeScale=1;mv.setAttribute('rotation-per-second','4deg')}catch{}};
  mv.addEventListener('pointerenter',()=>{try{mv.timeScale=1.18;mv.setAttribute('rotation-per-second','6deg')}catch{}});
...
bindV81Viewer(q('#plugyModel'));bindV81Viewer(q('#plugyFloatModel'));
