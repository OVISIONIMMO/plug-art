...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v84_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v84_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v84_history',JSON.stringify(A.history))}catch{}}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v84_last_opp',A.lastOppId)}catch{}
}
...
function bindV84Viewer(mv){
  if(!mv||mv.dataset.v84Bound==='1')return;mv.dataset.v84Bound='1';
...
bindV84Viewer(q('#plugyModel'));bindV84Viewer(q('#plugyFloatModel'));
