...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v89_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v89_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v89_history',JSON.stringify(A.history))}catch{}}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v89_last_opp',A.lastOppId)}catch{}
}
...
function bindV89Viewer(mv){
  if(!mv||mv.dataset.v89Bound==='1')return;mv.dataset.v89Bound='1';
...
bindV89Viewer(q('#plugyModel'));bindV89Viewer(q('#plugyFloatModel'));
})();