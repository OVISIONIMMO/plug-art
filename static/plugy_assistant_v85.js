...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v85_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v85_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v85_history',JSON.stringify(A.history))}catch{}}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v85_last_opp',A.lastOppId)}catch{}
}
...
function localAction(message){
  ...
  P.view(v);
  const label={dashboard:'l’accueil',radar:'le Radar',opencalls:'les Open Calls',studio:'le Studio Social',map:'la carte internationale',artists:'les profils artistes',crm:'le CRM'}[v];
...
async function askAI(msg){
  ...
  const crm=P.state.crm||[],artists=P.state.artists||[];const ui='Section actuelle: '+(document.body.dataset.view||location.hash.slice(1)||'dashboard')+(last?'\nDernière opportunité manipulée: '+last.title+' ('+[last.city,last.country].filter(Boolean).join(', ')+')':'')+'\nCRM: '+crm.length+' contact(s). Artistes: '+artists.length+' profil(s).';
...
function bindV85Viewer(mv){
  if(!mv||mv.dataset.v85Bound==='1')return;mv.dataset.v85Bound='1';
  ...
  mv.addEventListener('load',()=>{try{if((mv.availableAnimations||[]).includes('IdleBlink')){mv.animationName='IdleBlink';mv.play({repetitions:Infinity})}}catch{};q('#plugyFloat')?.classList.add('is-ready')},{once:true});
}
bindV85Viewer(q('#plugyModel'));bindV85Viewer(q('#plugyFloatModel'));
