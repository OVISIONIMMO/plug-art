...
function setMode(m){A.mode=m;document.body.dataset.plugyMode=m;const f=q('#plugyFloat');if(f)f.dataset.mode=m;dispatchEvent(new CustomEvent('plugy:mode',{detail:{mode:m,state:A.state}}))}
function syncMode(){
  if(A.modeRAF)return;
  A.modeRAF=requestAnimationFrame(()=>{
    A.modeRAF=0;
    const v=document.body.dataset.view||location.hash.slice(1)||'dashboard';
    const mode=A.conversation?'assistant':(v==='dashboard'&&scrollY<560?'hero':'mini');
    setMode(mode);
...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v90_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v90_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v90_history',JSON.stringify(A.history))}catch{}}
...
function openPanel(){q('#plugyChat')?.classList.add('open');document.body.dataset.plugyPanel='open';const was=A.conversation;A.conversation=true;setMode('assistant');if(!was){setState('expanding');setTimeout(()=>{if(A.conversation&&!A.busy)setState('idle')},480)}}
function closePanel(){q('#plugyChat')?.classList.remove('open');delete document.body.dataset.plugyPanel;stopConversation(false);A.conversation=false;syncMode();setState(A.mode==='mini'?'minimized':'idle')}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v90_last_opp',A.lastOppId)}catch{}
}
...
function localAction(message){
  ...
  if(has(/\b(connecte|connecter|connexion|meta)\b.{0,22}\b(instagram|insta|meta)\b/)){P.view('social');setTimeout(()=>window.PLUGInstagram?.open?.('setup'),220);return 'J’ouvre le centre de connexion Meta pour Instagram.'}
  if(has(/\b(ouvre|affiche|montre)\b.{0,18}\b(feed)\b.{0,14}\b(instagram|insta)?\b/)){P.view('social');setTimeout(()=>window.PLUGInstagram?.open?.('feed'),180);return 'J’ouvre ton feed Instagram dans le tableau de bord.'}
  if(has(/\b(ouvre|affiche|montre)\b.{0,18}\b(commentaires?|comments?)\b/)){P.view('social');setTimeout(()=>window.PLUGInstagram?.open?.('comments'),180);return 'J’ouvre la gestion des commentaires Instagram.'}
  if(has(/\b(ouvre|affiche|montre|planning|planifie|planifier)\b.{0,18}\b(instagram|insta|publication|posts?)\b/)){P.view('social');setTimeout(()=>window.PLUGInstagram?.open?.('planner'),180);return 'J’ouvre le planning Instagram.'}
  if(has(/\b(publie|publier|poste|poster)\b.{0,20}\b(instagram|insta)\b/)){P.view('social');setTimeout(()=>{window.PLUGInstagram?.open?.('planner');const b=document.querySelector('[data-social-publish]');flash(b)},240);return 'J’ouvre la file Instagram. Confirme la publication sur le bouton Publier pour éviter un envoi accidentel.'}
  ...
  if(has(/\b(interface lab|builder|design system|refonte|interface)\b/)){P.view('builder');return 'J’ouvre Interface Lab pour travailler la structure et PLUGY 2.0.'}
  if(has(/\b(instagram|insta|reseaux sociaux|réseaux sociaux)\b/)&&!has(/\b(analyse|analyser|optimise|optimiser|performance|strategie|stratégie|conseille|conseiller|pourquoi|compare|comparaison)\b/)){P.view('social');return 'J’ouvre le centre Instagram.'}
  ...
  else if(has(/\b(interface lab|builder|design system|refonte)\b/))v='builder';
...
const anchors={dashboard:[0,0],radar:[-10,-18],opencalls:[-18,-8],studio:[-30,-16],social:[-20,-10],map:[-16,-12],artists:[-20,-10],crm:[-12,-14],builder:[-24,-12]};
...
function bindV90Viewer(mv){
  if(!mv||mv.dataset.v90Bound==='1')return;mv.dataset.v90Bound='1';
...
bindV90Viewer(q('#plugyModel'));bindV90Viewer(q('#plugyFloatModel'));
})();