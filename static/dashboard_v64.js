...
/* ===== V64 visual-shell hooks ===== */
(function(){
  const labels={dashboard:["Vue générale","PLUG ART interne"],radar:["Open Calls","Radar & vérification"],studio:["Création","Studio carrousel"],network:["Réseau","Artistes, événements, lieux"],workspace:["Suivi","Actions & contacts"]};
  const refreshTitle=()=>{const id=(location.hash||"#dashboard").slice(1)||"dashboard";const m=labels[id]||labels.dashboard;const a=document.getElementById("pageTitle"),b=document.getElementById("pageSub");if(a)a.textContent=m[0];if(b)b.textContent=m[1]};
  document.querySelectorAll("[data-view]").forEach(btn=>btn.addEventListener("click",()=>setTimeout(refreshTitle,0)));
  addEventListener("hashchange",refreshTitle);refreshTitle();
  const quick=document.getElementById("quickGo"),legacy=document.getElementById("globalSearch");if(quick&&legacy)quick.onclick=()=>legacy.click();
  const homeAi=document.getElementById("homeAiStudio");if(homeAi)homeAi.onclick=()=>{view("studio");setTimeout(()=>document.getElementById("plugyCarouselBtn")?.click(),120)};
  const oldRender=renderDashboard;
  renderDashboard=function(){
    oldRender();
    const ds=store.get("plugart_v63_drafts",store.get("plugart_v62_drafts",[]));
    const first=ds[0];
    const t=document.getElementById("dashDraftTitle"),m=document.getElementById("dashDraftMeta");
    if(t)t.textContent=first?(first.name||first.slides?.[0]?.title||"Carrousel"):"Carrousel";
    if(m)m.textContent=first?((first.slides?.length||0)+" slides · "+(first.date||"brouillon")):"aucun brouillon";
    const av=document.getElementById("dashArtists");
    if(av)av.innerHTML=state.artists.slice(0,7).map(a=>'<span class="avatar" title="'+esc(a.name)+'">'+esc(initials(a.name))+'</span>').join("");
  };
})();
