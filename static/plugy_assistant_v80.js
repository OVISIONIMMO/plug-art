...
const q=P.q;
const A={state:'idle',mode:'hero',conversation:false,recognition:null,micStream:null,audioContext:null,analyser:null,raf:0,speaking:false,busy:false,longPress:0,history:[],voice:null,lastOppId:null,sleepTimer:0,modeRAF:0,thinkRAF:0,speakRAF:0,speakPulse:0,fxReady:false};
...
function syncMode(){
  ...
  A.modeRAF=requestAnimationFrame(()=>{
    ...
    const mode=A.conversation?'assistant':(v==='dashboard'&&scrollY<170?'hero':'mini');
    setMode(mode);
...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v80_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v80_last_opp')||null}catch{}
function remember(role,text){A.history.push({role,text:String(text).slice(0,1600)});A.history=A.history.slice(-8);try{sessionStorage.setItem('plugy_v80_history',JSON.stringify(A.history))}catch{}}
...
function speak(text){
  return new Promise(resolve=>{
    ...
    speechSynthesis.cancel();stopSpeakEnergy();
    const u=new SpeechSynthesisUtterance(String(text).replace(/\[\[[\s\S]*?\]\]/g,'').trim().slice(0,520));
    u.lang='fr-FR';u.rate=1.12;u.pitch=1.02;if(A.voice)u.voice=A.voice;
    u.onstart=()=>{A.speaking=true;setState('speaking');startSpeakEnergy();bumpSpeech(.38)};
    u.onboundary=e=>{const len=(e.charLength||1),boost=Math.min(.9,.26+len*.045);bumpSpeech(boost)};
    u.onpause=()=>{A.speakPulse=.08};
    u.onresume=()=>bumpSpeech(.34);
    u.onend=()=>{A.speaking=false;stopSpeakEnergy();resolve()};
    u.onerror=()=>{A.speaking=false;stopSpeakEnergy();resolve()};
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v80_last_opp',A.lastOppId)}catch{}
}
...
function localAction(message){
  ...
  else if(has(/\b(reseau|artistes?|evenements?)\b/))v='network';
...
async function askAI(msg){
  ...
  const last=(P.state.opps||[]).find(o=>String(o.id)===String(A.lastOppId));
  const ui='Section actuelle: '+(document.body.dataset.view||location.hash.slice(1)||'dashboard')+(last?'\nDernière opportunité manipulée: '+last.title+' ('+[last.city,last.country].filter(Boolean).join(', ')+')':'');
  const prompt=['Tu es PLUGY, assistant interne de PLUG ART.','Réponds en français, naturellement, de façon concise et opérationnelle.','Pour la voix, donne l’essentiel en 1 à 3 phrases quand cela suffit.','Ne prétends jamais avoir effectué une action du site si elle n’a pas été réellement déclenchée par l’interface.',ui,ctx?'Contexte récent:\n'+ctx:'','Nouvelle demande: '+msg].filter(Boolean).join('\n\n');
...
function bindV26Viewer(mv){
  if(!mv||mv.dataset.v26Bound==='1')return;mv.dataset.v26Bound='1';
  const normal=()=>{try{mv.timeScale=1;mv.setAttribute('rotation-per-second','5deg')}catch{}};
  mv.addEventListener('pointerenter',()=>{try{mv.timeScale=1.28;mv.setAttribute('rotation-per-second','8deg')}catch{}});
  mv.addEventListener('pointerleave',normal);
  mv.addEventListener('pointerdown',()=>{try{mv.timeScale=1.5}catch{}});
  mv.addEventListener('pointerup',()=>{try{mv.timeScale=1.05}catch{}});
  mv.addEventListener('load',()=>{try{if((mv.availableAnimations||[]).includes('IdleBlink')){mv.animationName='IdleBlink';mv.play({repetitions:Infinity})}}catch{}},{once:true});
}
bindV26Viewer(q('#plugyModel'));bindV26Viewer(q('#plugyFloatModel'));
