...
function smartLayout(render=true){
  const layouts=['cover','left','top','collage','collage3','grid4','frame','poster','right','minimal','text','artfield','band'],cuts=['none','diagonal','curve','wave'],themes=['editorial','glass','impact','paper','color','night'];
...
function renderStudio(){
  ...
  q('#artPhoto').style.backgroundImage=s.image?'url("'+s.image+'")':'linear-gradient(135deg,#2255ff,#7b70d6,#f08e51)';q('#artPhoto').style.backgroundPosition=s.position||'center';q('#artPhotoSecondary').style.backgroundImage=s.image2?'url("'+s.image2+'")':(s.image?'url("'+s.image+'")':'linear-gradient(135deg,#f1bf45,#7b70d6)');q('#artPhotoThird').style.backgroundImage=s.image3?'url("'+s.image3+'")':'linear-gradient(135deg,#e55d8e,#2255ff)';q('#artPhotoFourth').style.backgroundImage=s.image4?'url("'+s.image4+'")':'linear-gradient(135deg,#121826,#f08e51)';
  art.className='carousel-art theme-'+(s.theme||'editorial')+' layout-'+(s.layout||'top')+' cut-'+(s.cut||'none')+' align-'+(s.align||'left')+' font-'+(s.font||'sans')+' accent-'+(s.accent||'violet')+' visual-'+(s.visualMode||'photo');
  art.style.setProperty('--title-scale',Number(s.titleScale||100)/100);art.style.setProperty('--body-scale',Number(s.bodyScale||100)/100);art.style.setProperty('--preview-zoom',state.zoom||1);const format=q('#studioFormat')?.value||'portrait';art.style.aspectRatio=format==='square'?'1 / 1':format==='story'?'9 / 16':'4 / 5';
  const vals={editKicker:s.kicker,editTitle:s.title,editBody:s.body,editCta:s.cta,editImage:(s.image||'').startsWith('/api/')?'':s.image||'',editImage2:s.image2||'',editImage3:s.image3||'',editImage4:s.image4||'',layoutSelect:s.layout||'top',cutSelect:s.cut||'none',fontSelect:s.font||'sans',imagePosition:s.position||'center',imagePrompt:s.prompt||'',titleSize:s.titleScale||100,bodySize:s.bodyScale||100,visualMode:s.visualMode||'photo'};
  Object.entries(vals).forEach(([id,v])=>{const e=q('#'+id);if(e)e.value=v});q('#titleSizeValue').textContent=(s.titleScale||100)+'%';q('#bodySizeValue').textContent=(s.bodyScale||100)+'%';
...
function bind(){
  ...
  [['editKicker','kicker'],['editTitle','title'],['editBody','body'],['editCta','cta'],['editImage','image'],['editImage2','image2'],['editImage3','image3'],['editImage4','image4'],['layoutSelect','layout'],['cutSelect','cut'],['fontSelect','font'],['imagePosition','position'],['imagePrompt','prompt'],['visualMode','visualMode']].forEach(([id,key])=>{q('#'+id).oninput=()=>{slide()[key]=q('#'+id).value;renderStudio()}});
  q('#titleSize').oninput=()=>{slide().titleScale=Number(q('#titleSize').value);renderStudio()};q('#bodySize').oninput=()=>{slide().bodyScale=Number(q('#bodySize').value);renderStudio()};qa('[data-align]').forEach(b=>b.onclick=()=>{slide().align=b.dataset.align;renderStudio()});qa('[data-theme]').forEach(b=>b.onclick=()=>{slide().theme=b.dataset.theme;renderStudio()});qa('[data-accent]').forEach(b=>b.onclick=()=>{slide().accent=b.dataset.accent;renderStudio()});
  ...
  q('#generateCaption').onclick=caption;q('#improveStudio').onclick=improve;q('#queueInstagramBtn').onclick=queueInstagram;q('#plugyCarouselBtn').onclick=plugyCarousel;q('#generateSlideImage').onclick=async()=>{try{await genImage(state.slide,q('#generateSlideImage'))}catch(e){alert('Génération image : '+e.message)}};q('#illustrateSlideBtn').onclick=async()=>{slide().visualMode='art';slide().prompt=slide().prompt||'Illustration abstraite contemporaine inspirée de la matière picturale, du geste, du collage éditorial et de l’espace d’exposition.';renderStudio();try{await genImage(state.slide,q('#illustrateSlideBtn'))}catch(e){alert('Illustration : '+e.message)}};q('#generateAllImagesBtn').onclick=generateAllImages;q('#saveDraftBtn').onclick=saveDraft;q('#exportSlideBtn').onclick=exportSlide;
...
function imagePrompt(s){const o=currentOpp(),loc=[o?.city,o?.country].filter(Boolean).join(', '),dna='Univers PLUG ART : art contemporain émergent, exposition collective, peinture, photographie, matière acrylique fluide, collage éditorial, installation, culture urbaine et espace de galerie. Direction visuelle premium, expérimentale mais sobre, lignes franches, profondeur réelle, aucune esthétique corporate ou banque d’images.';return String(dna+' '+(s.prompt||('Sujet : '+s.title+'. Contexte : '+s.body+'. '))+(loc?' Lieu : '+loc+'. ':'')+' Sans texte lisible, sans logo, sans watermark, sans interface, sans mockup.').replace(/\s+/g,' ').trim()}
async function genImage(i,button){const s=state.slides[i],old=button?.textContent,format=q('#studioFormat')?.value||'portrait',ratio=format==='square'?'1:1':format==='story'?'9:16':'4:5';if(button){button.disabled=true;button.textContent='Création…'}try{const r=await api('/api/v32/content/image',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:imagePrompt(s),style:q('#imageStyle').value||'gallery',ratio,quality:q('#imageQuality').value||'medium'})});if(r.url){s.image=r.url;s.visualMode='art';if(i===state.slide)renderStudio()}}finally{if(button){button.disabled=false;button.textContent=old}}}
...
async function renderSlideBlob(index=state.slide){
  ...
  const noMedia=s.visualMode==='none'||s.layout==='text',mediaH=noMedia?0:Math.round(H*(format==='story'?.36:.385)),titleY=(noMedia?Math.round(H*.18):mediaH+Math.round(H*.09)),bodyY=titleY+Math.round(H*(format==='story'?.16:.20)),footerY=H-Math.round(H*.065);
  if(!noMedia)await drawMedia(x,s,0,0,W,mediaH);
