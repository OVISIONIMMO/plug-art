...
const q=P.q;
const A={state:'idle',mode:'hero',conversation:false,recognition:null,micStream:null,audioContext:null,analyser:null,raf:0,speaking:false,busy:false,longPress:0,history:[],voice:null,lastOppId:null,sleepTimer:0,modeRAF:0};
...
try{A.history=JSON.parse(sessionStorage.getItem('plugy_v78_history')||sessionStorage.getItem('plugy_v77_history')||'[]').slice(-8)}catch{}
try{A.lastOppId=sessionStorage.getItem('plugy_v78_last_opp')||null}catch{}
...
function rememberOpp(id){
  ...
  A.lastOppId=String(found.id);try{sessionStorage.setItem('plugy_v78_last_opp',A.lastOppId)}catch{}
}
...
resetSleep();setState('idle');window.PlugyAssistant={start:startListening,stop:stopConversation,ask:handle,setState,setMode,state:A,openOpportunity,createCarouselFor,filterOpenCalls,rememberOpp};
})();