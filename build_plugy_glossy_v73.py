...
def _patch_material_extensions(raw):
    ...
    for material in gltf.get("materials",[]):
        ...
        if "pearl" in name:
            ...
            ext["KHR_materials_clearcoat"]={"clearcoatFactor":1.0,"clearcoatRoughnessFactor":0.018}
            ext["KHR_materials_iridescence"]={
                "iridescenceFactor":0.72,
                "iridescenceIor":1.28,
                "iridescenceThicknessMinimum":125.0,
                "iridescenceThicknessMaximum":520.0
            }
        elif "eye" in name:
            used.add("KHR_materials_clearcoat")
            ext["KHR_materials_clearcoat"]={"clearcoatFactor":0.82,"clearcoatRoughnessFactor":0.022}
    gltf["extensionsUsed"]=sorted(used)
    return _pack_glb(gltf,blob)

def build_plugy_glossy_v73(output_path):
    pearl=mat((253,253,255),"PLUGY Pearl White",metallic=.035,rough=.060)
    pearl_back=mat((247,246,255),"PLUGY Pearl Back",metallic=.030,rough=.085)
    eye_left=mat((13,70,196),"PLUGY Eye Blue",emissive=(2,18,68),metallic=.14,rough=.035)
    eye_right=mat((37,22,120),"PLUGY Eye Violet",emissive=(10,3,48),metallic=.15,rough=.032)
    cyan=mat((83,229,255),"PLUGY Cyan Rim",emissive=(12,102,150),metallic=.015,rough=.10)
    ...
    _child(scene,superellipsoid((1.78,1.07,.72),4.85,pearl_back,62,40),"RearPearl",t=(0,.58,-.055))
    _child(scene,superellipsoid((.14,.72,.50),3.9,cyan,30,20),"CyanEdge",t=(-.825,.49,-.025))
    _child(scene,superellipsoid((.14,.72,.50),3.9,pink,30,20),"PinkEdge",t=(.825,.49,-.040))
    _child(scene,superellipsoid((.88,.11,.46),3.7,violet,34,16),"VioletBottom",t=(0,.055,-.035))
    _child(scene,superellipsoid((1.70,1.02,.68),5.00,pearl,68,42),"Head",t=(0,.57,.075))

    for x,side in [(-.36,"L"),(.36,"R")]:
        _child(scene,capsule_y(.108,.56,pearl,36),f"Prong{side}",t=(x,1.325,.020))

    for x,side,material in [(-.35,"L",eye_left),(.35,"R",eye_right)]:
        root=f"Eye{side}Root"; _group(scene,root,t=(x,.54,0))
        _arc(scene,root,.40,.132,.420,.060,material,f"Eye{side}",20)
    ...
    return {
      ...
      "design":"plugy-glossy-cute-pearl-head-v73",
      "body":False,"material":"pearl-white-ultra-clearcoat-iridescence","canonical":True
    }
